...
export default function AccessibilityContextProvider({ children }) {
  ...
  const [userHasHandicap, setUserHasHandicap] = useState(
    localStorage.getItem("userHasHandicap") === "true" ? true : false
  );
  const [userHasVisualHandicap, setUserHasVisualHandicap] = useState(
    localStorage.getItem("userHasVisualHandicap") === "true" ? true : false
  );
  const [titleColor, setTitleColor] = useState(
    localStorage.getItem("titleColor") ?? "#577eeb"
  );
